...
const SearchHistoryPage = () => {
	...
	useEffect(() => {
		const getSearchHistory = async () => {
			try {
				const res = await axios.get(`https://netflix-jxi6.onrender.com/api/v1/search/history`);
	...
	const handleDelete = async (entry) => {
		try {
			await axios.delete(`https://netflix-jxi6.onrender.com/api/v1/search/history/${entry.id}`);
